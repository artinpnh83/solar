...
<section class="bg-light py-5 position-relative overflow-hidden">
	...
	<div class="container position-relative">
		<div class="row g-4 align-items-center">
			<div class="col-md-6">
				<!-- Title -->
				<h1>{{ __('messages.blogs') }} {{ __('messages.in') }} {{ __('messages.sealban') }}</h1>
				<p>{{ __('messages.blogP') }}</p>
			</div>
...
<section class="py-5">
	<div class="container">
	    	    <div class="row">
			<div class="col-12">
				<div class="px-4 text-center rounded-3">
				...
				<div class="col-sm-6 col-lg-4 col-xl-3">
					<div class="card bg-transparent">
						<div class="overflow-hidden rounded-3">
							<img src="{{asset('storage/'.$blog->image)}}" style="height: 270px;" class="card-img">
							<!-- Overlay -->
							<div class="bg-overlay bg-dark opacity-4"></div>
							<div class="card-img-overlay d-flex align-items-start p-3">
								<!-- badge -->
								<a href="{{route('blog_detail', ['id'=>$blog->id])}}" class="badge text-bg-danger">{{$blog->category->name ?? ''}}</a>
							</div>
						</div>
						...
						<div class="card-body">
							<!-- Title -->
							<h5 class="card-title fw-normal"><a href="{{route('blog_detail', ['id'=>$blog->id])}}">{{$blog->title}}</a></h5>
							<p class="text-truncate-2">{{$blog->short_des}}</p>
							<!-- Info -->
							<div class="d-flex justify-content-between">
								<h6 class="mb-0 fw-normal"><a href="{{route('blog_detail', ['id'=>$blog->id])}}">{{ __('messages.sealban') }}</a></h6>
								<span class="small">
								    @if(app()->getLocale() == 'fa')
									{{ \Morilog\Jalali\Jalalian::forge($blog->updated_at)->format("%d %B %Y") }}
									@else
