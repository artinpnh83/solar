...
<section class="bg-light py-5 position-relative overflow-hidden">
	...
	<div class="container position-relative">
		<div class="row g-4 align-items-center">
			<div class="col-md-6">
				<!-- Title -->
				<h1>{{ __('messages.services') }} {{ __('messages.in') }} {{ __('messages.sealban') }}</h1>
				<p>{{ __('messages.serviceP') }}</p>
			</div>

			<div class="col-md-6 text-center">
				<!-- Image -->
				<img src="{{asset('assets/images/solar-services.png')}}" class="h-400px h-xl-500px" alt="">
			</div>
		</div>
	</div>
</section>
				...
				@if(count($service))
					...
					<div class="col-sm-6 col-lg-4 col-xl-3">
						<div class="card shadow h-100">
							<!-- Image -->
							<img src="/storage/{{$ser->image}}" class="card-img-top" style="height: 270px;" alt="course image">
							<!-- Card body -->
							<div class="card-body pb-0">
								<!-- Badge and favorite -->
								{{-- <div class="d-flex justify-content-between mb-2">
									<a href="/service_detail/{{$ser->id}}" class="badge bg-purple bg-opacity-10 text-purple">{{$ser->category->name}}</a>
								</div> --}}
								<!-- Title -->
								<h5 class="card-title fw-normal"><a href="/service_detail/{{$ser->id}}">{{$ser->title}}</a></h5>
							    <p>{!! $ser->short_des !!}</p>
							</div>
